...
export function Dashboard() { // Export as named function
    ...
    const renderMetricValue = (value: number | string, format?: "currency" | "number"): React.ReactNode => {
        ...
        return <div className="text-2xl font-bold">{typeof value === 'number' ? value.toLocaleString() : value}</div>;
    };
    ...
    const renderMetricChange = (changePercentage: number | null, trend: string): React.ReactNode => {
        ...
        const trendClass =
            trend === "increase" ? "text-green-600" : trend === "decrease" ? "text-red-600" : "text-muted-foreground";
